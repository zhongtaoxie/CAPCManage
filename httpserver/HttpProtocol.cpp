...
bool CHttpProtocol::StartHttpSrv()
{
	...
	{
		// 异常处理
		LOG_INFO("Could not create listen socket");
	...
	// 显示web服务器正在启动
	LOG_INFO("****** My WebServer is Starting now! *******");
...
UINT CHttpProtocol::ListenThread(LPVOID param)
{
	...
	{	
		...
		LOG_INFO("%s Connecting on socket:%d", inet_ntoa(SockAddr.sin_addr), socketClient);

        pReq = new REQUEST;
		if (pReq == NULL)
		{   
			// 处理错误
			LOG_ERROR("No memory for request");
			continue;
		}
		...
		{  
			// 线程创建失败,错误处理
			LOG_ERROR("Couldn't start client thread");
	...
	{  
		// 超时返回，并且同步对象未退出
		LOG_ERROR("One or more client threads did not exit");
	}
...
UINT CHttpProtocol::ClientThread(LPVOID param)
{
	...
	// 分析request信息
	g_pCADlg->m_parsermsg.Analyze(pReq,buf);
	return 0;
}
...
{
	CParserPostMsg parsermsg;
	return (parsermsg.Analyze(pReq, pBuf));
	...
	std::string szAction = "/?action=login";

	if (0 == szAction.compare(cpToken))
	{
		//std::string szResp;
		//SendLoginResp(pReq,szResp);

		CHttpProtocol *pHttpProtocol = (CHttpProtocol *)pReq->pHttpProtocol;
		SendMessage(pHttpProtocol->m_hwndDlg, LOGIN_MSG, (WPARAM)pReq, NULL);
	}
...
// 发送头部
void CHttpProtocol::SendLoginResp(PREQUEST pReq,std::string szResp)
{
...
void CHttpProtocol::SendFile(PREQUEST pReq)
{
 ...
 //  
	...
	{
		LOG_ERROR("Error occurs when closing file");
	}
}
...
{   
	...
	if (nRet != 0)
	{
		...
		{	
			...
			LOG_ERROR("WSASend() error: %d", WSAGetLastError() );
	...
	{	
		...
		{
			// 错误处理
			LOG_ERROR("WSAGetOverlappedResult() error: %d", WSAGetLastError() );
			CloseHandle(over.hEvent);
			return false;
		}
	}
...
void CHttpProtocol::Disconnect(PREQUEST pReq)
{
	// 关闭套接字：释放所占有的资源
	int	nRet;
	LOG_INFO("Closing socket: %d", pReq->Socket);
...
void CHttpProtocol::StopHttpSrv()
{
	...
	{
		LOG_ERROR("TIMEOUT waiting for ListenThread");
	}
	CloseHandle(m_hExit);
